...
export default class App extends Component {
  ...
  handleNumbers = (number)=> {
    this.setState((prevState)=> {
      if(prevState.output.includes("=")){
        ...
        return {
          // default case 
          display:prevState.display.toString().replace(/^0+/, '')+ number.toString(),
  ...
  handleOperator = (operator)=> {
    this.setState((prevState)=>{
      if(prevState.display.slice(-1)===operator && !prevState.output.includes("=")){
        ...
        return{
          display: operator,
          output: prevState.output.slice(prevState.output.indexOf("=")+1) + operator
        }
      }else if 
        ((prevState.output.endsWith("/")||prevState.output.endsWith("*")||prevState.output.endsWith("-")||prevState.output.endsWith("+"))){
        //if the output contains '+ or / or * or-' at end >> replace it with the new operator
          if((operator!=="-")&&(!isNaN(prevState.output.charAt(prevState.output.length-2)))){
            return{
              display: prevState.display.replace(prevState.display.charAt(prevState.display.length-1), operator),
              output: prevState.output.replace(prevState.output.charAt(prevState.output.length-1), operator)
            }
          }
          if((operator==="-")){
            return{
  ...
  doingMath = ()=> {
    ...
    // so we reduce it to only 10 places if it has 
    if(result % 1 !== 0){
      ...
      <Container 
        ...
        <Container 
          ...
          <Row className='flex-column text-end pt-2 pb-1'>
            <Col style={{minHeight:24}} className='text-warning px-1' id='output'>
              <p
                className='m-0'
